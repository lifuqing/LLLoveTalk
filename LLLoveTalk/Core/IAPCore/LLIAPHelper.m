...
#import "NSString+LLIAPHelper.h"
#import "SFHFKeychainUtils.h"

#if ! __has_feature(objc_arc)
#error You need to either convert your project to ARC or add the -fobjc-arc compiler flag to LLIAPHelper.m.
#endif


@interface LLIAPHelper()
@property (nonatomic,copy, nullable) IAPProductsResponseBlock requestProductsBlock;
@property (nonatomic,copy, nullable) resoreProductsCompleteResponseBlock restoreCompletedBlock;
@property (nonatomic,copy, nullable) checkReceiptCompleteResponseBlock checkReceiptCompleteBlock;
...
- (id)initWithProductIdentifiers:(NSSet *)productIdentifiers {
    if ((self = [super init])) {
        ...
        for (NSString * productIdentifier in _productIdentifiers) {
            ...
            NSString* password = [SFHFKeychainUtils getPasswordForUsername:productIdentifier andServiceName:@"LLIAPHelper" error:nil];
...
-(BOOL)isPurchasedProductsIdentifier:(NSString*)productID
{
    ...
    NSString* password = [SFHFKeychainUtils getPasswordForUsername:productID andServiceName:@"LLIAPHelper" error:nil];
    if([password isEqualToString:@"YES"])
    {
        productPurchased = YES;
    }
...
- (void)provideContentWithTransaction:(SKPaymentTransaction *)transaction {
    ...
    if (productIdentifier) {
        [SFHFKeychainUtils storeUsername:productIdentifier andPassword:@"YES" forServiceName:@"LLIAPHelper" updateExisting:YES error:nil];
...
- (void)clearSavedPurchasedProductByID:(NSString*)productIdentifier {
    ...
    [SFHFKeychainUtils deleteItemForUsername:productIdentifier andServiceName:@"LLIAPHelper" error:nil];
...
- (void)checkReceipt:(NSData*)receiptData AndSharedSecret:(NSString * _Nullable )secretKey onCompletion:(checkReceiptCompleteResponseBlock)completion
{
    ...
    NSString *receiptBase64 = [NSString ll_base64StringFromData:receiptData];
    ...
    if(conn) {
        ...
        error = [NSError errorWithDomain:@"LLIAPHelperError" code:100 userInfo:errorDetail];
