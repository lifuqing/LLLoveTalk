...
- (instancetype)initWithDiameter:(NSUInteger)diameter {
    return [self initWithDiameter:diameter cornerRadius:diameter/2.0];
}

- (instancetype)initWithDiameter:(NSUInteger)diameter cornerRadius:(CGFloat)cornerRadius
{
...
//        UIBezierPath *imgPath = [UIBezierPath bezierPathWithOvalInRect:frame];
        UIBezierPath *imgPath = [UIBezierPath bezierPathWithRoundedRect:frame cornerRadius:MAX(_cornerRadius, 0)];
        [imgPath addClip];
        [image drawInRect:frame];

        if (highlightedColor != nil) {
            CGContextSetFillColorWithColor(context, highlightedColor.CGColor);
            CGContextAddPath(context, imgPath.CGPath);
