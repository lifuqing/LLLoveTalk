...
- (void)layoutSubviews {
    ...
    CGFloat top = IS_IPhoneXSeries ? SafeNavgBarAreaTop : NormalStatusBarHeight;
    self.backButton.frame = CGRectMake(8, top, 30, NormalNavBarHeight);
...
- (UIButton *)backButton {
    if (!_backButton) {
        _backButton = [UIButton ll_buttonWithFrame:CGRectZero target:self normalImage:LLImage(@"btn_fanhui") selector:@selector(backButtonActionClick:)];
    }
    return _backButton;
}
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        _titleLabel = [UILabel ll_labelWithFrame:CGRectZero text:@"" font:LLTheme.navigationTitleFont textColor:LLTheme.navigationTitleColor textAlign:NSTextAlignmentCenter];
